...
- [2026-08-22] マジックナンバー（`86400000`・`3600000`・`100` 等の数値リテラル）は必ず意味を表す定数に切り出すこと（例: `const ONE_DAY_MS = 86_400_000`）。ファイル先頭またはモジュールスコープに定義する
...
- [2026-08-22] bug-fix-agent が PR を作成する際は必ず `Closes #<number>` を PR 本文に含めること（Issue のクローズは PR マージ時に自動で行う）
...
- [2026-08-22] 音声コマンド機能（VoiceService）で使用するClaudeモデルは `claude-haiku-4-5-20251001` を使用すること（旧: `claude-3-5-haiku-20241022`）
