...
- `src/voice/` — 音声コマンド（マルチターン対応）。`POST /voice/command`（JwtAuthGuard適用）で音声認識テキストを受け取り、Claude API（`@anthropic-ai/sdk`・モデル: claude-haiku-4-5-20251001）で意図解析して `{ action, params, reply, needs_followup, collected_params, final_params }` 形式のJSONを返す。action種別: `navigate`（画面遷移）・`create_task`（タスク作成）・`complete_task`（タスク完了）・`create_event`（予定作成）・`cancel`（キャンセル）・`unknown`（認識不能）。`create_task`・`create_event` では `needs_followup: true` のとき追加ヒアリングが必要で、フロントはリクエストに `context`（`action` + `collected_params`）を付けて再送する（ステートレス設計、最大2往復）。フロントエンドは `useVoiceCommand` フック（`frontend/src/hooks/useVoiceCommand.ts`）と `Sidebar.tsx` のマイクボタンで操作する（Web Speech API・lang: ja-JP）。フォローアップ中は発話完了後（`SpeechSynthesisUtterance.onend`）にマイクを自動再起動する
